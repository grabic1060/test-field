...
export function getTierTransactionFee(amount, tierLevel) {
  ...
  const normalizedTierLevel = Number.isInteger(tierLevel) ? tierLevel : Number.parseInt(tierLevel, 10);
  const safeIndex = Number.isInteger(normalizedTierLevel) && normalizedTierLevel >= 0
    ? Math.min(normalizedTierLevel, feeRates.length - 1)
    : 0;

  const rate = feeRates[safeIndex];
